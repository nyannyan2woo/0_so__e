...
function queries.createInventoryItem(shopId, item)
    return MySQL.insert.await([[
        INSERT INTO lation_shops_inventory (
            shop_id, item_name, label, description, category, icon, price, base_price, stock, max_stock, display_order, metadata, license, job_restriction
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ]], {
        shopId,
        item.item,
...
function queries.updateInventoryItem(shopId, itemName, data)
    return MySQL.update.await([[
        ...
        SET label = ?, description = ?, category = ?, icon = ?, base_price = ?, price = ?, stock = ?, metadata = ?, license = ?, job_restriction = ?
        WHERE shop_id = ? AND item_name = ?
    ]], {
        data.label,
        data.description,
