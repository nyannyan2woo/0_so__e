...
function ShowNotification(message, type)
    if shared.setup.notify == 'lation_ui' then
        exports.lation_ui:notify({ title = 'コンビニエンスストア', message = message, type = type, icon = 'fas fa-store' })
    ...
    elseif shared.setup.notify == 'okok' then
        exports['okokNotify']:Alert('コンビニエンスストア', message, 5000, type, false)
    elseif shared.setup.notify == 'sd-notify' then
        exports['sd-notify']:Notify('コンビニエンスストア', message, type)
    elseif shared.setup.notify == 'wasabi_notify' then
        exports.wasabi_notify:notify('コンビニエンスストア', message, 5000, type, false, 'fas fa-store')
...
function PoliceDispatch(data)
    ...
    if shared.police.dispatch == 'cd_dispatch' then
        ...
        TriggerServerEvent('cd_dispatch:AddNotification', {
            ...
            title = '10-88 - 店舗強盗',
            message = '24/7で警報が作動しました。場所: ' ..playerData.street,
            flash = 0,
    ...
    elseif shared.police.dispatch == 'ps-dispatch' then
        local alert = {
            coords = data.coords,
            message = '24/7で警報が作動しました。場所: ' ..data.street,
            dispatchCode = '10-88',
            description = '店舗強盗',
            radius = 0,
            sprite = 52,
    ...
    elseif shared.police.dispatch == 'core_dispatch' then
        ...
        TriggerServerEvent('core_dispatch:addCall', '10-88', '店舗強盗の可能性',
    ...
    elseif shared.police.dispatch == 'rcore_dispatch' then
        ...
        local alert = {
            ...
            coords = playerData.coords,
            job = shared.police.jobs,
            text = '24/7で警報が作動しました。場所: ' ..playerData.street_1,
            type = 'alerts',
            blip_time = 30,
            blip = {
                sprite = 52,
    ...
    elseif shared.police.dispatch == 'aty_dispatch' then
        TriggerEvent('aty_dispatch:SendDispatch', '店舗強盗の可能性', '10-88', 52, shared.police.jobs)
    elseif shared.police.dispatch == 'op-dispatch' then
        local job = 'police'
        local text = '24/7で警報が作動しました。場所: ' ..data.street
        local coords = data.coords
        local id = cache.serverId
        local title = '10-88 - 店舗強盗'
        local panic = false
    ...
    elseif shared.police.dispatch == 'emergencydispatch' then
        TriggerServerEvent('emergencydispatch:emergencycall:new', 'police', '10-88 | 店舗強盗の可能性', data.coords, true)
    elseif shared.police.dispatch == 'lb-tablet' then
        local lb_alert = {
            priority = 'high',
            code = '10-88',
			title = '店舗強盗',
			description = '24/7で警報が作動しました。場所: ' ..data.street,
			location = { label = '店舗強盗', coords = data.coords },
			time = 300,
			job = 'police',
        }
