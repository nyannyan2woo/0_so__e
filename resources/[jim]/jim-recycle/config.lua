...
Config = {
	...
	Main = {
		useLineHighlight = true,		-- リサイクル対象のアウトラインを表示します。QuantVでユーザーがクラッシュする場合はfalseに設定してください
		useblipTarget = true,			-- リサイクル対象の小さなマーカーを表示します。無効にするにはfalseに設定してください
	},
	Crafting = {
		showItemBox = true,
	},
...
Config.RecyclingCenter = {
	EnableOpeningHours = true, -- 有効にすると、プレイヤーは1日の特定の時間のみアクセス可能
	OpenHour = 9, -- リサイクルセンターはいつから営業しますか？ ( 9 = 午前9時 )
	CloseHour = 21, -- リサイクルセンターはいつ営業終了しますか？ ( 21 = 午後9時 )

	PayAtDoor = nil, -- nilに設定して倉庫入場料を無効にします

	TradeForRandomItems = false,	-- trueに設定すると、リサイクル素材を取引時にランダムに選別された素材を受け取ります

	propTable = { -- スポーンするクレートのテーブル
...
Config.Other = {
	...
	RecycleAmounts = {		-- 1回のリサイクルボックス投入で獲得できる量
		["Recycle"] = {
			Min = 2,
			Max = 5,
		},
...
-- ロケール用の関数
-- 内容を理解している場合のみ変更してください
-- これはすべてのものが読み込まれる前に読み込まれる必要があるためここに配置されます
function locale(section, string)
    if not Config.Lan or Config.Lan == "" then
        print("^1Error^7: ^3Config^7.^3Lan ^1が設定されていません^7、^2Config.Lan = 'en'にフォールバックしています")
        Config = Config or {}
        Config.Lan = "en"
    end

    local localTable = Loc[Config.Lan]
    -- Loc[..] が存在しない場合、ユーザーに警告
    if not localTable then
		print("ロケールテーブル '"..Config.Lan.."' が見つかりません")
        return "ロケールテーブル '"..Config.Lan.."' が見つかりません"
    end

    -- Loc[..].section が存在しない場合、ユーザーに警告
    if not localTable[section] then
		print("^1Error^7: ロケールセクション: ['"..section.."'] 無効です")
        return "ロケールセクション: ['"..section.."'] 無効です"
    end

    -- Loc[..].section.string が存在しない場合、ユーザーに警告
    if not localTable[section][string] then
		print("^1Error^7: ロケール文字列: ['"..section.."']['"..string.."'] 無効です")
        return "ロケール文字列: ['"..section.."']['"..string.."'] 無効です"
    end
