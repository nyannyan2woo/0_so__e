...
local function createGarage(vehicles, coords)
    lib.zones.box({
        ...
        onEnter = function()
            if QBX.PlayerData.job.type == 'ems' and QBX.PlayerData.job.onduty then
                exports.lation_ui:showText({
                    description = locale('text.veh_button'),
                    keybind = 'E',
        ...
        onExit = function()
            local _, text = exports.lation_ui:isOpen()
            if text == locale('text.veh_button') then exports.lation_ui:hideText() end
        end,
        inside = function()
    ...
    CreateThread(function()
        for i = 1, #sharedConfig.locations.duty do
            lib.zones.box({
                ...
                onExit = function()
                    local _, text = exports.lation_ui:isOpen()
                    if text == locale('text.onduty_button') or text == locale('text.offduty_button') then exports.lation_ui:hideText() end
                end,
                inside = function()
                    if QBX.PlayerData.job.type ~= 'ems' then return end
        ...
        for i = 1, #sharedConfig.locations.stash do
            lib.zones.box({
                ...
                onEnter = function()
                    ...
                    exports.lation_ui:showText({
                        description = locale('text.pstash_button'),
                        keybind = 'E',
                        icon = 'fas fa-hand-paper'
                    })
                    end,
                onExit = function()
                    local _, text = exports.lation_ui:isOpen()
                    if text == locale('text.pstash_button') then exports.lation_ui:hideText() end
                end,
                inside = function()
                    if QBX.PlayerData.job.type ~= 'ems' then return end
                    OnKeyPress(function()
            ...
            for ii = 1, #sharedConfig.locations.armory[i].locations do
                lib.zones.box({
                    ...
                    onEnter = function()
                        if QBX.PlayerData.job.type ~= 'ems' or not QBX.PlayerData.job.onduty then return end
                        exports.lation_ui:showText({
                            description = locale('text.armory_button'),
                            keybind = 'E',
                            icon = 'fas fa-hand-paper'
                        })
                        end,
                    onExit = function()
                        local _, text = exports.lation_ui:isOpen()
                        if text == locale('text.armory_button') then exports.lation_ui:hideText() end
                    end,
                    inside = function()
                        if QBX.PlayerData.job.type ~= 'ems' then return end
                        OnKeyPress(function()
        ...
        lib.zones.box({
            ...
            onExit = function()
                local _, text = exports.lation_ui:isOpen()
                if text == locale('text.elevator_main') or text == locale('error.not_ems') then exports.lation_ui:hideText() end
            end,
            inside = function()
        ...
        lib.zones.box({
            ...
            onExit = function()
                local _, text = exports.lation_ui:isOpen()
                if text == locale('text.elevator_roof') or text == locale('error.not_ems') then exports.lation_ui:hideText() end
            end,
            inside = function()
