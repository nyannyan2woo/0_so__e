    ...
    if config.useTarget then
        ...
        local zone = lib.zones.box({
            ...
            onEnter = function()
                if QBX.PlayerData.job.onduty and QBX.PlayerData.job.type == 'mechanic' then
                    exports.lation_ui:showText({
                        description = locale('labels.o_stash'),
                        keybind = 'E',
                        icon = 'fa fa-archive'
                    })
                end
    ...
    lib.zones.box({
        ...
        onEnter = function()
            if QBX.PlayerData.job.onduty and QBX.PlayerData.job.type == 'mechanic' then
                ...
                if inVehicle then
                    exports.lation_ui:showText({
                        description = locale('labels.h_vehicle'),
                        keybind = 'E',
                        icon = 'fa-solid fa-warehouse'
                    })
                else
                    exports.lation_ui:showText({
                        description = locale('labels.g_vehicle'),
                        keybind = 'E',
                        icon = 'fa-solid fa-warehouse'
                    })
                end
...
local function registerVehiclePlateZone(id, plate)
    ...
    local plateZone = lib.zones.box({
        ...
        onEnter = function()
            ...
            if plate.AttachedVehicle then
                exports.lation_ui:showText({
                    description = locale('labels.o_menu'),
                    keybind = 'E',
                    icon = 'fa-solid fa-wrench'
                })
            elseif cache.vehicle then
                exports.lation_ui:showText({
                    description = locale('labels.work_v'),
                    keybind = 'E',
                    icon = 'fa-solid fa-wrench'
                })
            end
