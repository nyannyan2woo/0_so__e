            ...
            if #(playerCoords - electricalCoords) <= 1.5 and not isHacking then
                ...
                elseif not config.useDrawText and not hasShownText then
                    ...
                    exports.lation_ui:showText({
                        description = locale('text.electrical'),
                        keybind = 'E',
        ...
        while true do
            ...
            if nearby and not (isSmashing or sharedConfig.vitrines[closestVitrine].isOpened) then
                ...
                elseif not config.useDrawText and not hasShownText then
                    hasShownText = true
                    exports.lation_ui:showText({
                        description = locale('text.cabinet'),
                        keybind = 'E',
                        icon = 'fas fa-hand-paper'
                    })
                end
                if IsControlJustPressed(0, 38) then
                    lib.callback('qbx_jewelery:callback:cabinet', false, function(CanSmash)
                        ...
                        isSmashing = true
                        if hasShownText then hasShownText = false exports.lation_ui:hideText() end
                        TriggerEvent('qbx_jewelery:client:cabinetHandler')
                    end, closestVitrine)
                end
            end
            if not nearby and hasShownText then hasShownText = false exports.lation_ui:hideText() end
            Wait(waitTime)
        end
    end)
end
