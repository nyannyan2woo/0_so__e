...
local function onStoreEnter(data)
    ...
    if jobName == (store.job or store.gang) then
        ...
        local textOptions = {
            clothing = { icon = "fas fa-tshirt", text = string.format(_L("textUI.clothing"), Config.ClothingCost) },
            barber = { icon = "fas fa-scissors", text = string.format(_L("textUI.barber"), Config.BarberCost) },
            tattoo = { icon = "fas fa-pen", text = string.format(_L("textUI.tattoo"), Config.TattooCost) },
            surgeon = { icon = "fas fa-scalpel", text = string.format(_L("textUI.surgeon"), Config.SurgeonCost) }
        }
        local option = textOptions[currentZone.name]
        if option then
...
local function onClothingRoomEnter(data)
    ...
    if jobName == (clothingRoom.job or clothingRoom.gang) then
        if CheckDuty() or clothingRoom.gang then
            ...
            exports.lation_ui:showText({
                description = _L("textUI.clothingRoom"),
                keybind = "E",
...
local function onPlayerOutfitRoomEnter(data)
    ...
    if isAllowed then
        ...
        exports.lation_ui:showText({
            description = _L("textUI.playerOutfitRoom"),
            keybind = "E",
            icon = "fa-solid fa-right-to-bracket"
        })
        Radial.AddOption(currentZone)
